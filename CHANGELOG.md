...
- **Multi-bucket routing** via the optional `pkg_bucket_map` secret parameter — a JSON object mapping Benchling folder IDs (`lib_…`) or project IDs (`src_…`) to S3 bucket names. An entry is routed by its folder first, then its project; unmapped entries fall back to `user_bucket`. Routing applies to exports, SQS packaging messages, canvas package links, and package-event canvas refreshes. Configured at runtime in AWS Secrets Manager (no redeploy); also settable via `packages.bucketMap` in the CLI profile config. Constraints: mapped buckets must already be attached to the Quilt stack (the task role's `BucketWritePolicy` and the per-bucket Athena `<bucket>_packages-view` only cover stack buckets); routing is evaluated per event, so moving an entry to a different folder re-routes *future* packaging while existing packages stay in the bucket they were written to
...
- **Async canvas refresh** - Quilt `package-revision` events refresh Benchling canvases automatically via EventBridge, with a two-state "pending → complete" UX
- **Review record events** - `v2.entry.updated.reviewRecord` now triggers the standard entry export workflow

### Fixed

- Canvas-initiated package exports now preserve `canvas_id` in `entry.json`
- Canvas updates use SDK `update_canvas` for reliable async rendering
