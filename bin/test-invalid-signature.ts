...
const testPayload: unknown = JSON.parse(readFileSync("test-events/app-installed.json", "utf8"));
const rawBody: string = JSON.stringify(testPayload);
...
const webhookId: string = "wh_test123";
const webhookTimestamp: string = Math.floor(Date.now() / 1000).toString();

// Sign with the WRONG private key
const payloadToSign: string = `${webhookId}.${webhookTimestamp}.${rawBody}`;
const signer = createSign("sha256");
signer.update(payloadToSign);
signer.end();

const invalidSignature: string = signer.sign(privateKey).toString("base64");
...
(async (): Promise<void> => {
    try {
        ...
        let responseBody: ExecutionResponse | string;
        try {
            responseBody = JSON.parse(responseText) as ExecutionResponse;
        } catch {
        ...
        if (response.status === 202) {
            ...
            if (typeof responseBody === "object" && responseBody.executionArn) {
                ...
                const executionArn: string = responseBody.executionArn;
