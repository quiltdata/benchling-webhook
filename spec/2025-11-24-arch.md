...
5. **VPC Requirements**: Default VPC no longer supported - requires private subnets with NAT Gateway

---

## VPC Requirements (v0.9.0)

The HTTP API + VPC Link architecture **requires private subnets with NAT Gateway**:

### Why Private Subnets?

- **VPC Link**: Connects API Gateway to private resources (ECS tasks in private subnets)
- **ECS with `assignPublicIp: false`**: Requires private subnets for secure container deployment
- **NAT Gateway**: Enables outbound internet access for ECS tasks (pulling images, AWS API calls)

### VPC Configuration Options

#### Option 1: Auto-Create VPC (Default)

If no VPC is specified in config, CDK creates a production-grade VPC:

- **2 Availability Zones** for high availability
- **Public subnets** for NAT Gateways and internet access
- **Private subnets** for ECS tasks (with NAT Gateway routes)
- **2 NAT Gateways** (one per AZ) - ~$64/month

**Architecture mirrors** `~/GitHub/deployment/t4/template/network.py` (network_version=2.0)

#### Option 2: Use Existing VPC

Specify VPC ID in profile configuration (`deployment.vpc.vpcId`).
...
**Validation**: Deployment fails if VPC lacks private subnets
